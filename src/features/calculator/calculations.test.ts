...
describe('calculateFootprint', () => {
  ...
  it('keeps valid decimal activities finite', () => {
    const result = calculateFootprint({ ...values, monthlyDistanceKm: 12.5, monthlyKwh: 140.75, monthlyOrganicWasteKg: 3.25 })
    expect(Number.isFinite(result?.monthlyTotalKg)).toBe(true)
    expect(result?.monthlyTotalKg).toBeGreaterThan(0)
  })
